...
{
    public class GetCrawlJobDataBehaviour : TollGroupProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(TollGroupConstants.KeyName.ApiKey, nameof(TollGroupCrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task TollGroupCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.TollGroup.Core.TollGroupCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
